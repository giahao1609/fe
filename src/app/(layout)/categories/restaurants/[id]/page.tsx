...
export const dynamic = "force-static"; // hoặc "force-dynamic" tuỳ ông

const SITE_URL =
  process.env.NEXT_PUBLIC_SITE_URL ?? "https://food-map.online";
const API_URL = process.env.NEXT_PUBLIC_BACKEND_URL;
...
function buildAddress(r?: RestaurantSeo | null) {
  if (!r?.address) return "";
  const { street, ward, district, city } = r.address;
  return [street, ward, district, city].filter(Boolean).join(", ");
}

async function fetchRestaurantForSeo(id: string): Promise<RestaurantSeo | null> {
  if (!API_URL) return null;
  try {
    // tuỳ API của ông, ví dụ /restaurants/public/:id
    const res = await fetch(`${API_URL}/restaurants/${id}`, {
      next: { revalidate: 60 }, // revalidate mỗi 60s
    });
    if (!res.ok) return null;
    const data = await res.json();
    // nếu backend bọc data: { data: {...} }
    return (data.data ?? data) as RestaurantSeo;
  } catch {
    return null;
  }
}
...
export async function generateMetadata(
  ...
  const restaurant = await fetchRestaurantForSeo(params.id);

  if (!restaurant) {
    const fallbackTitle = "Nhà hàng | FoodMap";
    const url = `${SITE_URL}/restaurants/${params.id}`;
    return {
      title: fallbackTitle,
      description:
        "Khám phá nhà hàng, xem thực đơn, đánh giá và định vị trên bản đồ cùng FoodMap.",
  ...
  const baseTitle = restaurant.metaTitle?.trim() || restaurant.name;
  const title = `${baseTitle} - Đặt bàn & xem thực đơn | FoodMap`;

  const descFromMeta = restaurant.metaDescription?.trim();
  const descFallbackParts = [
    `Nhà hàng ${restaurant.name}`,
    address ? `tại ${address}` : "",
    restaurant.categoryName ? `, chuyên ${restaurant.categoryName}` : "",
    ". Xem thực đơn, đánh giá, hướng dẫn đường đi và đặt bàn trực tuyến trên FoodMap.",
  ];
  const description =
    descFromMeta ||
    descFallbackParts
  ...
  const pathSegment = restaurant.slug
    ? `restaurants/${restaurant.slug}`
    : `restaurants/${params.id}`;
  const url = `${SITE_URL}/${pathSegment}`;

  const image =
    restaurant.coverImageUrl ||
    restaurant.logoUrl ||
    `${SITE_URL}/images/og-default-restaurant.png`;
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: `Hình ảnh nhà hàng ${restaurant.name}`,
        },
      ],
    },
    twitter: {
...
export default function Page() {
  // client component tự fetch lại để hiển thị UI
