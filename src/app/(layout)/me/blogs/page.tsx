// src/app/(layout)/owner/blogs/OwnerBlogsTab.tsx
"use client";
...
export default function OwnerBlogsTab() {
  ...
  const [heroExistingUrl, setHeroExistingUrl] = useState<string | null>(null);

  // gallery (chỉ new file, gallery cũ chỉ xem preview)
  ...
  const heroPreviewUrl = useMemo(() => {
    ...
    return heroExistingUrl;
  }, [heroFile, heroExistingUrl]);
  ...
  const startEdit = (b: BlogPost) => {
    ...
    setHeroFile(null);
    setHeroExistingUrl(getHeroUrl(b));

    const urlsFromSigned =
      Array.isArray(b.gallerySigned) && b.gallerySigned.length
        ? b.gallerySigned
            .map((g) => g.url || null)
            .filter((x): x is string => !!x)
        : [];
    const urlsFromPlain =
      !urlsFromSigned.length && Array.isArray(b.gallery) ? b.gallery : [];

    setExistingGalleryUrls(
      urlsFromSigned.length ? urlsFromSigned : urlsFromPlain,
    );
    setGalleryItems([]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        await BlogService.updateById(editingId, payload, uploadFiles);
      } else {
        await BlogService.create(payload, uploadFiles);
      }
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-6">
          <div>
            <h2 className="text-xl font-semibold text-gray-900 flex items-center gap-2">
              {editingId ? "Chỉnh sửa blog" : "Đăng blog mới"}
              {editingId && (
          ...
          {editingId && (
            <button
              type="button"
              onClick={resetForm}
              className="text-xs rounded-full border border-gray-300 px-3 py-1 text-gray-600 hover:border-gray-400"
            >
              Hủy chỉnh sửa
            </button>
          )}
            ...
            <div className="border border-dashed rounded-xl p-3 bg-gray-50/60">
              ...
              >
                ...
                <div className="flex flex-col items-center gap-1">
                  ...
                  <p className="text-[11px] text-gray-400">
                    Có thể kéo để thay đổi thứ tự hiển thị (đối với ảnh mới)
                  </p>
                </div>
              </div>
              ...
              {!!existingGalleryUrls.length && (
                <div className="mt-3">
                  <p className="text-[11px] text-gray-500 mb-1">
                    Ảnh gallery hiện đang có:
                  </p>
                  <div className="flex flex-wrap gap-2">
                    {existingGalleryUrls.slice(0, 12).map((src, idx) => (
                      <div
                        key={`${src}-${idx}`}
                        className="h-16 w-16 rounded-lg overflow-hidden border border-gray-200"
                      >
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img
                          src={src}
              ...
              {/* Gallery mới (kéo đổi thứ tự, có nút xóa) */}
              {!!galleryItems.length && (
                <>
                  ...
                  <p className="mt-1 text-[11px] text-gray-400">
                    {galleryItems.length} ảnh mới sẽ được upload theo thứ tự
                    đang hiển thị.
                  </p>
                </>
              )}
